...
import { collection, query, getDocs, where, doc, updateDoc, setDoc } from "firebase/firestore";
...
export function PmsView({ currentUid, setActiveTab }: PmsViewProps) {
  ...
  useEffect(() => {
    if (!currentUid) return;
    const q = query(collection(db, `users/${currentUid}/pmsSchedules`));
    getDocs(q).then(snap => {
      if (snap.empty) {
        ...
        const parseAndUpload = async () => {
          ...
          for (const task of defaultTasks) {
            await setDoc(doc(collection(db, `users/${currentUid}/pmsSchedules`)), {
              ...task,
              schedDate: task.schedDate ? task.schedDate.toISOString() : null,
              actualDate: task.actualDate ? task.actualDate.toISOString() : null,
            });
          }
          const resnap = await getDocs(q);
          const tasks = resnap.docs.map(d => {
            const data = d.data();
            return {
              ...data,
              id: d.id,
              schedDate: data.schedDate ? new Date(data.schedDate) : null,
  ...
  const allTasks = useMemo(() => {
    let tasks = [...pmsTasks, ...customTasks];
      ...
      if (t.activity.toLowerCase().includes("flushing")) {
        linkedActivity = completedActivities.find((act) => {
          ...
          const sameMonth =
            t.schedDate &&
            act.date &&
            new Date(act.date).getMonth() === t.schedDate.getMonth();
  ...
  const handleSync = async () => {
    try {
      ...
      const confirmed = window.confirm(
        "Are you sure you want to sync the PMS schedule to your Google Calendar? This will create new events.",
      );
      if (!confirmed) return;
    ...
    try {
      if (currentUid && selectedTask) {
        ...
        const matchingAreaDoc = areaSnap.docs.find(d => {
          ...
          if (areaData.name && selectedTask.pumpStation.toLowerCase().includes(areaData.name.toLowerCase())) return true;
          // also check if any sites match
          if (areaData.sites && areaData.sites.some((s: string) => selectedTask.pumpStation.toLowerCase().includes(s.toLowerCase()))) return true;
          return false;
        });
        ...
        if (matchingAreaDoc) {
          ...
          if (selectedTask.wellCode) {
            updates[`lastServicedByFacility.${selectedTask.wellCode}`] = new Date().toISOString();
          }
          await updateDoc(areaRef, updates);
        }

        // Just mock saving the activity since PMS tasks are largely static parsed data
        // in a real app we'd update a "status" field on the PMS task doc in FireStore here.
      }
      ...
      console.error(e);
      showToast("Error updating Facility Profile", "error");
    } finally {
  ...
  const getTasksForDay = (day: number) => {
    return allTasks.filter(
      (t) =>
        t.schedDate &&
        t.schedDate.getDate() === day &&
        t.schedDate.getMonth() === currentMonth.getMonth() &&
        t.schedDate.getFullYear() === currentMonth.getFullYear(),
    );
  };
    ...
    <div className="max-w-5xl mx-auto px-margin-mobile pt-lg md:pt-xl mb-24 relative">
      ...
      <section className="mb-lg flex flex-col md:flex-row md:items-end md:justify-between gap-md">
        ...
        <div className="flex gap-2 flex-wrap items-center md:justify-end">
          <button
            ...
            className="bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 px-4 py-2 rounded-lg transition-all shadow-sm font-label-md flex items-center gap-2"
          >
            <CloudUpload className="w-4 h-4" /> Import Schedules
          </button>
          <button className="bg-surface hover:bg-surface-variant text-primary border border-outline-variant px-4 py-2 rounded-lg transition-all shadow-sm font-label-md hidden md:block">
            Export Logs
          </button>
          <button
            onClick={handleSync}
              ...
              <div className="grid grid-cols-7 gap-1">
                ...
                {daysInMonth.map((day) => {
                  ...
                  const isOverdue = dayTasks.some(
                    (t) =>
                      t.schedDate! < new Date(2026, 5, 15) && !t.actualDate,
                  );
                        ...
                        <div className="mt-1 flex flex-col gap-1 w-full px-1">
                          {dayTasks.map((t) => (
                            <div
                              key={t.id}
                              title={t.activity}
                              className={`text-[10px] md:text-xs font-semibold truncate px-1 py-0.5 rounded ${t.actualDate ? "bg-success/20 text-success" : t.schedDate! < new Date(2026, 5, 15) ? "bg-error/20 text-error" : "bg-primary/10 text-primary"}`}
                            >
                              {t.activity}
                            </div>
                          ))}
            ...
            <div className="p-4 bg-surface-container-low border-b border-outline-variant flex justify-between items-center shrink-0">
              <div>
                <h3 className="font-headline-md text-on-surface">
                  {selectedTask.activity || "PMS Task"}
                </h3>
                <p className="text-label-sm text-on-surface-variant flex items-center gap-1 mt-1">
                  <AlertTriangle className="w-3 h-3" />{" "}
                  {selectedTask.pumpStation} &bull;{" "}
                  {selectedTask.schedDate?.toLocaleDateString()}
                </p>
              </div>
              <button
                  ...
                  {selectedTask.linkedActivity ? (
                    <div className="p-4 border border-outline-variant rounded-lg bg-surface-container-lowest">
                      <p className="text-sm font-medium mb-2 opacity-70">
                        Activity logged on{" "}
                        {new Date(
                          selectedTask.linkedActivity.date,
                        ).toLocaleDateString()}
                      </p>
                ...
                <div className="space-y-sm">
                  ...
                  <div className="flex flex-col gap-xs">
                    ...
                    <input
                      ...
                      className="rounded bg-white text-on-surface focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent border border-outline-variant p-3 font-body-md"
                    />
                  </div>
          ...
          <div className="bg-surface w-full max-w-[448px] rounded-2xl shadow-lg flex flex-col">
            ...
            <div className="p-4 border-t border-outline-variant bg-surface-container-low flex justify-end gap-3 rounded-b-2xl">
              ...
              <button
                onClick={async () => {
                  if (
                    !newScheduleForm.pumpStation ||
                    !newScheduleForm.schedDate
                  )
                    return;
                  ...
                  const taskObj = {
                    ...
                    wellCode: newScheduleForm.wellCode,
                    activity: newScheduleForm.activity,
                    remarks: "",
                    schedDate: new Date(parseInt(y), parseInt(m) - 1, parseInt(d)).toISOString(),
                    actualDate: null,
                  };

                  try {
                    await setDoc(doc(collection(db, `users/${currentUid}/pmsSchedules`), newId), taskObj);
                    
                    const newTask: PmsTask = {
                      id: newId,
                      ...taskObj,
                      schedDate: new Date(taskObj.schedDate),
                      actualDate: null
                    };

                    setPmsTasks([...pmsTasks, newTask]);
      ...
      {isImporting && (
        <div className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center p-4 sm:p-6 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-[90vw] sm:w-[512px] max-w-[512px] rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-4 flex justify-between items-center border-b border-outline-variant bg-surface-container-low">
              <h3 className="font-headline-sm flex items-center gap-2 text-on-surface">
                <CloudUpload className="w-5 h-5 text-primary" /> Import Custom
                Schedules
              </h3>
              <button
            ...
            <div className="p-6 overflow-y-auto">
              <p className="text-body-md text-on-surface-variant mb-6">
                Upload a CSV file to bulk import preventive maintenance
                schedules. Please ensure your CSV follows the exact format
                required.
              </p>
              ...
              <div className="bg-surface-container-low border border-outline-variant rounded-lg p-4 mb-6">
                ...
                <div className="font-mono text-xs text-on-surface bg-surface-container-lowest p-2 rounded border border-outline-variant overflow-x-auto whitespace-nowrap">
                  Area/Plant,Site/Village/Phase,Well/Tank/Site&Block,Activity,Scheduled
                  Date,Remarks
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 mt-8">
                <button
                  onClick={() => {
                    const csvContent =
                      "Area/Plant,Site/Village/Phase,Well/Tank/Site&Block,Activity,Scheduled Date (YYYY-MM-DD),Remarks\n";
                    ...
                    link.setAttribute("download", "PMS_Schedule_Template.csv");
                  ...
                  className="flex justify-center items-center gap-2 py-2 px-4 border border-outline-variant rounded-lg text-primary hover:bg-surface-container font-label-md transition-colors bg-white"
                >
                  Download Template
                </button>
